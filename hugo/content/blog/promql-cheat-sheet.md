...
metrics where you want to see quick changes (like CPU usage).
{{< /tip >}}
...
```
Use for counters that always increase. Handles counter resets.

**`irate()` - Instant rate (last 2 data points)**
```promql
irate(http_requests_total[5m])
```
More sensitive to short-term spikes. Good for CPU metrics.
...
| `min()` | Minimum value | `min(rate(http_requests_total[5m]))` | Lowest rate of throughput for each contanier or pod
| `max()` | Maximum value | `max(rate(http_requests_total[5m]))` | Highest rate of throughput for each container or pod
...
- alert: DiskWillFillIn4Hours
  ...
  annotations:
    ...
    description: "Filesystem {{ $labels.mountpoint }} will run out of space in 4 hours"
```
...
{{< tip title="Aggregate before rate" >}}
When possible, aggregate metrics before calculating rates for better performance.
{{< /tip >}}

{{< warning title="Avoid high cardinality" >}}
Don't use labels with unbounded values (user IDs, timestamps, etc.). This causes performance issues.
{{< /warning >}}
...
```promql
# Sum across all pods in a deployment
sum by(deployment) (rate(requests_total[5m]))

# Average CPU per namespace
avg by(namespace) (container_cpu_usage)

# Max memory per node
max by(node) (container_memory_usage_bytes)
